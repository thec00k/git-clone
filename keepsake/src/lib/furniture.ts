...
export type FurnitureId=`${FurnitureCategory}-${1|2|3}`;
export type FurnitureChoices=Partial<Record<FurnitureCategory,FurnitureId>>;
export type RoomFurnitureChoices=Partial<Record<'woodland'|'beachfront'|'cyberpunk'|'snowy-mountain',FurnitureChoices>>;
...
const collection:Partial<Record<FurnitureCategory,string[]>>={...names,desk:[...names.desk,'Sculpted bentwood desk'],chair:[...names.chair,'Honey shell swivel chair'],lamp:[...names.lamp,'Graphite articulated lamp']};
export const FURNITURE_ITEMS=Object.entries(collection).flatMap(([category,titles])=>titles.map((title,i)=>({id:`${category}-${i+1}` as FurnitureId,category:category as FurnitureCategory,title,asset:`/room/furniture/${category}-${i+1}.glb${category==='chair'&&i===2?'?v=padded-3':''}`,thumbnail:`/room/furniture/${category}-${i+1}.png`})));
export function validFurnitureChoices(value:unknown):value is RoomFurnitureChoices {
  if(!value||typeof value!=='object'||Array.isArray(value))return false;
  return Object.entries(value).every(([room,choices])=>['woodland','beachfront','cyberpunk','snowy-mountain'].includes(room)&&!!choices&&typeof choices==='object'&&!Array.isArray(choices)&&Object.entries(choices).every(([category,id])=>FURNITURE_ITEMS.some(item=>item.category===category&&item.id===id)));
}
