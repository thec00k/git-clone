...
export type RoomFurnitureChoices=Partial<Record<'woodland'|'beachfront'|'cyberpunk',FurnitureChoices>>;
...
export function validFurnitureChoices(value:unknown):value is RoomFurnitureChoices {
  ...
  return Object.entries(value).every(([room,choices])=>['woodland','beachfront','cyberpunk'].includes(room)&&!!choices&&typeof choices==='object'&&!Array.isArray(choices)&&Object.entries(choices).every(([category,id])=>FURNITURE_ITEMS.some(item=>item.category===category&&item.id===id)));
}
