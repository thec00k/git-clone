...
export const FURNITURE_ITEMS=Object.entries(collection).flatMap(([category,titles])=>titles.map((title,i)=>({id:`${category}-${i+1}` as FurnitureId,category:category as FurnitureCategory,title,asset:`/room/furniture/${category}-${i+1}.glb`,thumbnail:`/room/furniture/${category}-${i+1}.png`})));
