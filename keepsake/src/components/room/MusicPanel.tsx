...
export function MusicPanel({onClose}:{onClose:()=>void}) {
 const ref=useRef<HTMLDivElement>(null);useFocusTrap(ref,onClose);
 return <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/50 p-4" onClick={onClose}><div ref={ref} className="ks-panel max-w-lg max-h-[90dvh] overflow-y-auto p-5" role="dialog" aria-modal="true" aria-label="Music" onClick={e=>e.stopPropagation()}><button onClick={onClose} aria-label="Close music"><X size={18}/></button><MusicSettings/></div></div>;
}
export function MusicSettings() {
 const {environment,setEnvironment,activeBook,setBookPlaylist}=useApp();const sp=useSpotify();const {setSlot}=useCrtPlayerSlot();
