...
import {binderCardAt,type CardBinder} from '../lib/cardBinders';
export function BinderPageTurn({binder,from,to,onDone}:{binder:CardBinder;from:number;to:number;onDone:()=>void}){
 ...
 const page=(index:number,ref:typeof front)=><div ref={ref} className="ks-pocket-page ks-binder-capture-page" style={{backgroundColor:binder.color}}>{Array.from({length:9},(_,i)=>{const c=binderCardAt(binder.cards,index*9+i);return <div className="ks-card-pocket" key={i}>{c?.src?<img src={c.src} alt=""/>:<span>{c?.modelSrc?c.title:index*9+i+1}</span>}</div>;})}</div>;
