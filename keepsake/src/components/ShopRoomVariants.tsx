...
const descriptions={'sky-castle':'Pearl-white stone, a circular sky window, crystal furniture and waterfalls above the clouds.',cyberpunk:'A neon skyline, holographic keepsakes and softly glowing lamps.',woodland:'Moss-painted wood, layered forest views and warm lamplight.',beachfront:'An arched window, pale plaster, sea-glass details and a quiet ocean view.'};
export function ShopRoomVariants(){
 ...
 return <><p className="mb-3 text-sm text-paper/65">A change of scenery. Your books and memories come with you. Rooms are included during this prototype.</p><ul className="ks-sticker-shop-list">{(['woodland','beachfront','cyberpunk','sky-castle'] as const).map(id=><li key={id} className="ks-sticker-pack"><div className="flex-1"><p className="font-display">{roomThemes[id].title}</p><small>{descriptions[id]}</small></div><button className="ks-tool" disabled={loading||isVisitor} onClick={()=>{setPreview(id);setNote('');}}>{current===id?'View current room':`Preview ${roomThemes[id].title}`}</button></li>)}</ul>
 {preview&&<section className="mt-4 p-3 border border-paper/20 rounded-lg" aria-label={`${roomThemes[preview].title} preview`}>
   <img className="w-full rounded-md mb-3" src={`/room/${preview}/preview.${preview==='sky-castle'?'png':'svg'}`} alt={`${roomThemes[preview].title} room preview`} onError={e=>{e.currentTarget.hidden=true;}}/>
 ...
 <p role="status" className="my-3 text-sm">{note}</p><p className="ks-handnote mt-4">Further along the coast…</p><ul className="ks-sticker-shop-list">{[['Snowy Mountain','A warm cabin above a snowy valley'],['Stormy Lighthouse Room','Stormy seas, rain-streaked glass and warm lantern light']].map(([name,description])=><li key={name} className="ks-sticker-pack"><div className="flex-1"><p className="font-display">{name}</p><small>{description}</small></div><span className="ks-sticker-owned">Coming later</span></li>)}</ul></>;
}
