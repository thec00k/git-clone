...
const descriptions={'snowy-mountain':'Cedar logs, a stone hearth, soft marbled fur and northern lights over snowy peaks.',cyberpunk:'A neon skyline, holographic keepsakes and softly glowing lamps.',woodland:'Moss-painted wood, layered forest views and warm lamplight.',beachfront:'An arched window, pale plaster, sea-glass details and a quiet ocean view.'};
export function ShopRoomVariants(){
 ...
 <p role="status" className="my-3 text-sm">{note}</p><p className="ks-handnote mt-4">Further along the coast…</p><ul className="ks-sticker-shop-list">{[['Stormy Lighthouse Room','Stormy seas, rain-streaked glass and warm lantern light']].map(([name,description])=><li key={name} className="ks-sticker-pack"><div className="flex-1"><p className="font-display">{name}</p><small>{description}</small></div><span className="ks-sticker-owned">Coming later</span></li>)}</ul></>;
}
