...
export function CardBinders({onClose}:{onClose:()=>void}) {
 const {state,update}=useApp();const root=useRef<HTMLDivElement>(null),alive=useRef(true);
 const binders=state.cardBinders??[];
 const [id,setId]=useState(binders[0]?.id??''),[spread,setSpread]=useState(0),[selected,setSelected]=useState<string|null>(null);
 ...
 const create=()=>{const b=newCardBinder();update(s=>({...s,cardBinders:[...(s.cardBinders??[]),b]}));setId(b.id);setSpread(0);setCover(true);setSelected(null);};
 ...
 return <div className="ks-binder-backdrop"><div ref={root} className="ks-binder-dialog" role="dialog" aria-modal="true" aria-labelledby="binder-heading">
  <header><div><small>A COLLECTION WORTH KEEPING</small><h2 id="binder-heading">Card binders</h2></div><button onClick={onClose} aria-label="Close card binders">×</button></header>
  <nav aria-label="Binder collection"><label>Collection <select aria-label="Binder collection" disabled={busy} value={binder?.id??''} onChange={e=>{setId(e.target.value);setSpread(0);setSelected(null);setCover(false);}}>{!binders.length&&<option value="">No binders yet</option>}{binders.map(b=><option key={b.id} value={b.id}>{b.title}</option>)}</select></label><button disabled={busy||binders.length>=12} onClick={create}>New card binder</button></nav>
  {!binder?<div className="ks-binder-empty"><h3>Your cards, gathered in one place.</h3><p>Create a binder for trading cards, postcards, or small pieces of your life.</p></div>:<>
   <div className="ks-binder-tools">{cover&&<label>Binder title <input maxLength={80} value={binder.title} onChange={e=>change(b=>({...b,title:e.target.value}))}/></label>}<button aria-pressed={cover} onClick={()=>{setCover(!cover);setSelected(null);}}>{cover?'Open binder':'View cover'}</button>{cover&&<label>Cover color <select aria-label="Cover color" value={binder.color} onChange={e=>change(b=>({...b,color:e.target.value}))}>{BINDER_COLORS.map((c,i)=><option value={c} key={c}>{['Moss','Ocean','Plum','Cognac','Charcoal'][i]}</option>)}</select></label>}</div>
   ...
   {cover?<div className="ks-binder-cover" style={{backgroundColor:binder.color}}>{binder.coverSrc&&<img src={binder.coverSrc} alt="Binder cover artwork"/>}<h3>{binder.title||'Untitled collection'}</h3><span>{binder.cards.length} keepsakes</span><label className="ks-binder-file">Choose cover artwork<input disabled={busy} type="file" accept="image/*" aria-label="Upload binder cover" onChange={e=>{void images(e.target.files,'cover');e.target.value='';}}/></label></div>:<div className="ks-card-spread" style={{'--binder-color':binder.color} as CSSProperties}>
    {[0,1].map(side=><section key={`${spread}-${side}`} className="ks-pocket-page" aria-label={`Binder page ${spread*2+side+1}`}>
     {Array.from({length:9},(_,slot)=>{const index=spread*18+side*9+slot;const c=binder.cards[index];return <button className={`ks-card-pocket ${c?.finish==='foil'?'is-foil':''}`} key={slot} disabled={!c} aria-label={c?`Inspect card ${index+1}: ${c.title}`:`Empty pocket ${index+1}`} onClick={()=>{setSelected(c!.id);setBack(false);}}>{c?.src?<img src={c.src} alt={c.title} loading="lazy"/>:c?.modelSrc?<span>◇<br/>{c.title}<small>3D scan</small></span>:<span className="ks-pocket-number">{index+1}</span>}</button>;})}
    </section>)}<div className="ks-binder-rings" aria-hidden="true"><i/><i/><i/></div>
   </div>}
   {!cover&&<footer><button disabled={!spread} onClick={()=>setSpread(s=>s-1)}>Previous binder spread</button><span>Spread {spread+1} of {pages} · {binder.cards.length} cards</span><button disabled={spread>=pages-1} onClick={()=>setSpread(s=>s+1)}>Next binder spread</button></footer>}
