...
export function TimeCapsuleChest(){
 const {environment}=useApp();const metal=environment.roomTheme==='cyberpunk'&&environment.capsuleFinish!=='wood';
 const {scene}=useGLTF(metal?'/room/furniture/time-capsule-metal.glb?v=c005d55ec36d':'/room/furniture/time-capsule.glb');const {isVisitor,setDiscoveryOpen}=useNav();
