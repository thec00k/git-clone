...
export function BeachfrontScenery({phase,environment}: {phase:Phase;environment:Environment}) {
  const reduced=useReducedMotion(); const waves=useRef<THREE.Group>(null); const boat=useRef<THREE.Group>(null);
  const night=phase==='night', dusk=phase==='dusk', storm=environment.weather!=='clear';
  const sky=night?'#182c43':storm?'#879fa8':dusk?'#e9ac8d':'#abd8df';
  const water=night?'#24485b':storm?'#5f929e':dusk?'#77a7ac':'#4fa5ae';
  ...
  useFrame(({clock},dt)=>{
    ...
    waves.current?.children.forEach((o,i)=>{o.position.z=-5-i*.9+Math.sin(t*.3+i)*.16;o.scale.x=1+Math.sin(t*.2+i)*.06;});
