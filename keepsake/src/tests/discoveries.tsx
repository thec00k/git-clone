...
function Verification(){
 ...
 }}>Prepare shelf reward fixture</button><button onClick={()=>setDiscoveryOpen('collection')}>Open test correspondence</button><button onClick={async()=>{await flushSave();const saved=await loadState();setResult(JSON.stringify(saved?.discoveries)===JSON.stringify(state.discoveries)?'PASS: discoveries match IndexedDB':'FAIL');}}>Verify discovery save</button><button onClick={()=>location.reload()}>Reload verification</button><button onClick={()=>update(s=>({...s,discoveries:{...discoveryState(s),frequency:'occasional',guestNotesOnDesk:true,entries:[...discoveryState(s).entries.filter(e=>e.keptAt&&e.id!=='test-guest'),{id:'test-guest',guestEntryId:'qa-guest',author:'River',title:'A note from River',text:'Thank you for a quiet afternoon. Let’s make another page soon.',delivery:'desk',location:'desk',appearedAt:Date.now()}]}}))}>Prepare guest postcard</button><audio controls preload="metadata" src="/audio/paper-slide.mp3" aria-label="Paper slide audio verification"/><button onClick={()=>{const frame=document.querySelector<HTMLIFrameElement>('iframe[title="Your SoundCloud player"]');const api=(window as unknown as {SC?:{Widget:(el:HTMLIFrameElement)=>{getDuration:(fn:(duration:number)=>void)=>void;seekTo:(time:number)=>void;play:()=>void}}}).SC;if(!frame||!api){setResult('Load a SoundCloud playlist first');return;}const w=api.Widget(frame);w.getDuration(duration=>{w.seekTo(duration-1000);w.play();setResult('Playing final second; watch next song');});}}>Verify automatic next song</button><output>{result}</output></aside></>;
}
