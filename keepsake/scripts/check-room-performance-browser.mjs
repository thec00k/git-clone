...
try{for(const [room,motion] of [['woodland','no-preference'],['beachfront','reduce']]){
 ...
 await p.routeWebSocket('**',()=>{}); const errors=[];p.on('pageerror',e=>errors.push(e.message));
 ...
 const canvas=p.locator('canvas').first();const b=await canvas.boundingBox();const beforeLook=await p.locator('.ks-room3d').getAttribute('data-look');await p.mouse.move(b.x+b.width*.5,b.y+b.height*.5);await p.mouse.down();await p.mouse.move(b.x+b.width*.5+70,b.y+b.height*.5,{steps:10});await p.mouse.up();await p.waitForTimeout(200);assert.notEqual(await p.locator('.ks-room3d').getAttribute('data-look'),beforeLook);
 console.log(`PASS ${room}: open/close, page turns, photo drag, drawing, saving, furniture reload and camera drag.`);await context.close();
