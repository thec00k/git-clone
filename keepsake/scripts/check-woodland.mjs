...
const budget = {...woodlandRoom.budget, maxBytes: process.argv.includes('--high') ? 12 * 1024 * 1024 : woodlandRoom.budget.maxBytes};
const theme = process.argv.includes('--beachfront') ? 'beachfront' : 'woodland';
const fileArg = process.argv.indexOf('--file');
const file = fileArg >= 0 ? process.argv[fileArg + 1] : new URL('../public/room/'+theme+'/'+theme+'.glb', import.meta.url);
...
assert.ok(bytes.length <= budget.maxBytes, 'Room within its graphics-quality download budget');
